...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False):
        ...
        textures_dir = os.path.join(self.current_dir, "textures")
        plates_dir = os.path.join(textures_dir, "_plates")
        
        # Get only PNG files from textures directory (exclude directories and system files)
        texture_list = sorted([f for f in os.listdir(textures_dir) if f.endswith('.png')])
        plates_list = sorted(os.listdir(plates_dir))
        
        # Select a random texture and use same index for corresponding plate image
        random_texture = random.choice(texture_list)
        random_texture_index = texture_list.index(random_texture)
        self.plate_image_path = os.path.join(plates_dir, plates_list[random_texture_index])
        self.textureId = p.loadTexture(os.path.join(textures_dir, random_texture))
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    robotId = p.loadURDF(os.path.join(self.current_dir, "ot_2_simulation_v6.urdf"), position, [0,0,0,1],
                    ...
                    planeId = p.loadURDF(os.path.join(self.current_dir, "custom.urdf"), position_with_offset, rotate_90)#start_orientation)
